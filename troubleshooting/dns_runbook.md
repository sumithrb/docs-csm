...
```bash
ncn-m001:~ # kubectl get pods -n services -l app.kubernetes.io/instance=cray-dns-unbound
...
```shell
ncn-m001:~ # kubectl get cronjob -n services -l app.kubernetes.io/instance=cray-dns-unbound
...
```shell
ncn-m001:~ # kubectl get cm -n services cray-dns-unbound -o yaml|grep forward-addr
...
Example:
```
ncn-w001:~ # kubectl logs -n services pod/$(kubectl get -n services pods | grep unbound | tail -n 1 | cut -f 1 -d ' ') -c manager | tail -n4
